  ...
  return <><Backdrop /><StructuredData data={{ "@context": "https://schema.org", "@type": "CreativeWork", name: project.name, description: project.description, url: absoluteUrl(`/work/${project.slug}`), image: absoluteUrl(project.image), genre: project.sector, creator: { "@id": `${siteConfig.url}/#organization` } }} /><Interactions /><Nav />
    <main id="main" className="internal-page overflow-hidden pt-24">
      <section className="pb-16 pt-16 sm:pb-24 sm:pt-24"><div className="container-x">
        ...
        <Reveal delay={130}><div className="mt-7 flex flex-col items-start justify-between gap-6 lg:flex-row lg:items-end"><p className="max-w-2xl text-lg leading-8 text-white/60">{project.description}</p><a href={project.liveUrl} target="_blank" rel="noopener noreferrer" className="rounded-full bg-[#991b3b] px-6 py-3.5 text-sm font-semibold text-[#ffffff] transition-transform hover:-translate-y-0.5">View live site ↗</a></div></Reveal>
      ...
      <CaseSection eyebrow="Project overview" title="What the project is."><div className="space-y-5">{project.overview.map((text) => <p key={text}>{text}</p>)}</div><div className="mt-8 rounded-2xl border border-white/10 bg-white/[.04] p-6"><p className="font-mono text-[.65rem] uppercase tracking-[.18em] text-[#ff9bb4]">Project role</p><p className="mt-3 font-display text-2xl font-semibold text-white">{project.role}</p></div></CaseSection>
      ...
      <CaseSection eyebrow="What was built" title="The real work behind it."><ul className="grid gap-3">{project.built.map((text) => <li key={text} className="rounded-xl border border-white/10 bg-white/[.04] px-5 py-4 text-white/65">{text}</li>)}</ul></CaseSection>
